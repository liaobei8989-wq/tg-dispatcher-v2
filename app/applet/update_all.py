...
"""
VPS 终极完整更新补丁：
1. 修复 tg_dispatcher.py 中的隐藏零宽字符与用户名解析穿透逻辑
2. 修复 SimplifiedTgHub.tsx 中的智能接力触发判断
3. 执行 npm run build（输出 1-1718 模块编译进度）
4. 重启 PM2 进程
"""
import os
import re
...
if os.path.exists(dispatcher_path):
    ...
    # 替换 send_single_target 中的用户名处理逻辑
    old_pattern = r"async def send_single_target.*?clean_target = target\.strip\(\).*?if clean_target\.startswith\('@'\).*?if not peer:.*?raise Exception\(f\"USERNAME_NOT_OCCUPIED:.*?\)"
...
if os.path.exists(hub_path):
    ...
    if "用户名不存在" in hub_content:
        ...
        with open(hub_path, "w", encoding="utf-8") as f:
            f.write(hub_content)
        print("✅ SimplifiedTgHub.tsx 智能接力判断规则已修复（不再将未匹配到的用户名误判为空号）！")
    else:
        print("✅ SimplifiedTgHub.tsx 已经是最新规则！")

print("⚡ 正在启动 Vite 生产级编译 (即将展示 1-1718 模块构建全过程)...")
sys.stdout.flush()
res = subprocess.run("npm run build", shell=True)
if res.returncode == 0:
    print("🎉 Vite 全量构建完成 (1718 个模块编译成功)！")
    print("🔄 正在重启 PM2 守护进程...")
    subprocess.run("pm2 restart all || true", shell=True)
    print("✨ 所有服务已重启就绪！请在浏览器按 Ctrl + F5 强制刷新后台！")
else:
    print(f"⚠️ npm run build 退出码: {res.returncode}")
